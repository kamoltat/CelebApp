...
export class HomePage implements OnInit {
  ...
  constructor(private navCtrl: NavController,
    ...
    private actionSheetCtrl: ActionSheetController, private alertCtrl: AlertController, private sanitizer: DomSanitizer) {
          ...
          handler: () => {
            ...
            postRef.once('value').then(snapshot => {
              ...
              if (childData.post_pic_url != "") {
                var picStorageRef = firebase.storage().ref(childData.post_pic_path);
                picStorageRef.delete();
              }
...
sortByTime(a,b){
  var one = new Date(a.timeStamp).getTime();
  var two = new Date(b.timeStamp).getTime();
  return one - two; 
}
